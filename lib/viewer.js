    ...
    multiImgDB.findOne({ _id: imageId }, (err, dir) => {
      ...
      imgDB.find({ _id: { $in: imgIdList } }, (err, imgs) => {
        ...
        $('#tag-add-button').on('click', () => {
          const nowImage = imgList[index];
          const newTag = $('#tag-input').val();
 ...
 */
function updateTagsToImage(imgId, tags) {
...
function changeImage(index) {
  ...
  const nextImage = imgList[index].path;
  ...
  if (pageMode == 2) {
    const nextImage = imgList[movePage(+1)].path;
