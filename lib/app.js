...
function setAddDrectoryEvent() {
  ...
  $('#add-directory').on('click', () => {
    ...
    let imageList = fileList.filter((file) => {
      return ImgUtil.isImage(file.path);
    });
    ...
    const dirList = fileList.filter((file) => {
      return !ImgUtil.isImage(file.path);
    });

    multiImgDB.insert(dirList, (err, newImages) => {
      let newImagesNum = 0;
      let message = 'No image added.';
...
function createDirIcon(image) {
  ...
  let tags = image.tags || [];
  let imgIcon;
  let dirImage = ImgUtil.firstImage(image.path);
