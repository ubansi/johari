...
function setAddDrectoryEvent() {
  ...
  $('#open-add-directory').on('click', openAddDirectoryModal);
    ...
    dirPathList.forEach((dir) => {
      ...
      dirImages.forEach((dirImage) => {
        if (ImgUtil.isImage(dirImage.path)) {
          dirImage.partOfMultiImage = true;
...
function addThumb(image) {
  ...
  let liClass = 'image-thumb';
  let multiHidenStyle = '';
  if(image.partOfMultiImage){
    liClass += ' partOfMultiImage';
    multiHidenStyle = 'style="display:none;"';
  }

  const tumbs = `<li class="${liClass}" ${multiHidenStyle}>
...
function createDirIcon(dir) {
  ...
  imgDB.findOne({ _id: dirImageId }, (err, img) => {
    ...
    if (thumbnail) {
      imgIcon = thumbDirIconHTML(img, dirName);
 ...
 * @param {string} img.thumbnail
 * @param {string} img._id
 * @param {string} dirName 
 */
function thumbDirIconHTML(img, dirName) {
  return `
  <img class="dir-thumb" src="${img.thumbnail}" data-thumbnail="${img._id}">
 ...
 * @param {{path :string,_id:string,thumbnail:string,partOfMultiImage:boolean}[]} images
 * @return {Promise} 
*/
        ...
        createThumbImg(image.path, image._id)
          .then((thumbnailPath) => {
            ...
            imgDB.update({ _id: image._id }, { $set: { thumbnail: thumbnailPath } }, {}, (err, updates) => {
              ...
              $(`img[data-thumbnail=${image._id}]`).attr('src',image.thumbnail);
            });
